...
if ($_GET['step']==6) {
	...
	$result_article = dbquery("SELECT
											article_id,
											article_content
									FROM ". DB_ARTICLES ."");
									// LIMIT 0, 10
	if (dbrows($result_article)) {
		$data_array = array();
		while ($data_article = dbarray($result_article)) {
			$article_content = unserialize($data_article['article_content']);
			$data_array[$data_article['article_id']] = $article_content[LOCALESHORT];
		} // db whille
	} // db query

	foreach ($data_array as $data_key => $data_value) {

		$data_value = htmlspecialchars_decode($article_content[LOCALESHORT]);
		$data_value = str_replace("<h4>", "<legend>", $data_value);
		$data_value = str_replace("</h4>", "</legend>", $data_value);
		$data_value = str_replace("</legend>", "</legend>\n<ul>", $data_value);
		$data_value = str_replace('<div class="product-char">', "<fieldset>", $data_value);
		$data_value = str_replace("</div>\n</div>", "</li>\n</ul>\n</fieldset>", $data_value);
		$data_value = str_replace('<div class="param">', "<li>", $data_value);
		$data_value = str_replace("</div>", "</li>", $data_value);
		$data_value = str_replace("</label>", ":</label>", $data_value);


		$data_value = stripinput($data_value);
		...
		// 														article_content='". serialize($data_value) ."'
	...
	if (dbrows($resultcat)) {
		while ($datacat = dbarray($resultcat)) {
			...
			$article_cat_title = "Холодильники ". $article_cat_name[LOCALESHORT] ." - характеристики, объем, программы, функции, No Frost, видео обзор";
			$article_cat_title = array(
											"ru"=>$article_cat_title
										);

			$article_cat_description = "В каталоге холодильники ". $article_cat_name[LOCALESHORT] ." вы можете ознакомиться с описанием, фотографиями и подробными техническими характеристиками.";
			$article_cat_description = array(
											"ru"=>$article_cat_description
										);
			
			$article_cat_h1 = "Холодильники ". $article_cat_name[LOCALESHORT];
			$article_cat_h1 = array(
											"ru"=>$article_cat_h1
										);
	...
	if (dbrows($resultarticle)) {
		while ($dataarticle = dbarray($resultarticle)) {
			...
			$article_title = "Холодильник ". $article_name[LOCALESHORT] ." - характеристика, объем, программы, функции, No Frost, видео обзор";
			$article_title = array(
											"ru"=>$article_title
										);
			
			$article_description = "В каталоге холодильник ". $article_name[LOCALESHORT] ." вы можете ознакомиться с описанием, фотографиями и подробными техническими характеристиками.";
			$article_description = array(
											"ru"=>$article_description
										);

			$article_h1 = "Холодильник ". $article_name[LOCALESHORT];
			$article_h1 = array(
											"ru"=>$article_h1
										);
	...
	$url = "https://market.yandex.ru/vendors.xml?CAT_ID=141694&hid=90578&track=fr_cm_vendor";
	...
	if (dbrows($resultarticle_cat)) {
		while ($dataarticle_cat = dbarray($resultarticle_cat)) {
			...
			$article_cat_array[$dataarticle_cat['article_cat_id']]['article_cat_status'] = $dataarticle_cat['article_cat_status'];
	...
	foreach ($all_cats_urls as $return1_key => $return1_value) {
		if (substr_count($return1_value['all_cats_url'], "guru.xml")) { $j_arts++;
			if ($j_arts<=99991) {
				...
				foreach ($article_cat_array as $cat_value) {
					if ($cat_value['article_cat_name']==$return1_value['all_cats_name']) {
						$get_cat_id = $cat_value['article_cat_id'];
						if ($cat_value['article_cat_status']==2) {
							$get_status = false;
						} // if article_cat_status
					} // if article_cat_name
				} // foreach article_cat_array
				...
				if ($get_status==true) {
					...
					for ($i_page=1; $i_page <= 999991; $i_page++) {
						...
						$url = "https://market.yandex.ru/catalog/". $catalog ."/list?hid=". $hid ."&gfilter=". $gfilter ."%3A". $markaid ."&in-stock=0". ($i_page>1 ? "&page=". $i_page : "");
					...
					if ($get_cat_id>0) {
						$update_cat_status = dbquery("UPDATE ". DB_ARTICLE_CATS ." SET article_cat_status='2' WHERE article_cat_id='". $get_cat_id ."'");
					} // if get_cat_id > 0
